...
for CONF in aliPublish*-rpms.conf; do
  ...
  printf "\n\n\n\n" >&2
  timeout 300 rclone sync --config /secrets/alibuild_rclone_config --transfers=10 local:/repo/RPMS/el7.x86_64/ rpms3:alibuild-repo/RPMS/el7.x86_64/ || true
  timeout 300 rclone sync --config /secrets/alibuild_rclone_config --transfers=10 local:/repo/UpdRPMS/el7.x86_64/ rpms3:alibuild-repo/UpdRPMS/el7.x86_64/ || true
done
